...
# Requires ffmpeg with libx264/libx265, ffprobe, and sha256sum.
set -eu
...
encode()
{
    ...
    actual=$(ffprobe -v error -f "$codec" -count_frames -select_streams v:0 \
        -show_entries stream=nb_read_frames -of default=nw=1:nk=1 "$OUT/$name.$ext")
...
encode h264 1920x1080 600
encode hevc 1920x1080 600
encode h264 3840x2160 300
encode hevc 3840x2160 300
(cd "$OUT" && sha256sum ./*.h264 ./*.h265 > SHA256SUMS)
